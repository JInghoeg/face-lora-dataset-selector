...
- [ ] 2.1 Extract the shared `ImagePreview` and `ThumbnailWorker` into reusable `ui/qt` components without semantic changes; verify existing Text Cleanup, Composite Review, and main Dataset call sites can use the shared modules.
- [ ] 2.2 Create `ui/qt/text_cleanup.py` containing `TextScan`, `TextCleanupBatchWorker`, and `SubtitleTab`; verify the module imports without importing feature internals or `app.py`.
- [ ] 2.3 Replace implicit `app.py.BACKEND`/presentation globals with explicit constructor dependencies while keeping calls on `SelectorApplication`; verify no backend feature package changes are required.
- [ ] 2.4 Update `app.py` and `ui/qt/__init__.py` to compose the moved presentation and remove the migrated class definitions; verify startup/self-test still resolves the Text Cleanup tab.
...
- [ ] 3.3 Run the existing Architecture Boundaries / Import Linter and UI regression checks; verify UI remains presentation -> SelectorApplication rather than bypassing the application facade.
