...
The Text Cleanup backend boundary is already complete, but its Qt workers, image preview widget, thumbnail worker, and `SubtitleTab` presentation still live inside the large legacy `app.py`. The accepted v0.3 plan is now in the UX/UI modernization phase, so this is a bounded brownfield slice that proves presentation can move independently without reopening backend or algorithm work.
...
- Move Text Cleanup Qt presentation and its thin Qt workers from `app.py` into `ui/qt/text_cleanup.py`.
- Make the presentation receive `SelectorApplication` and presentation/cache paths explicitly instead of depending on the `app.py` global `BACKEND`.
...
- Update `app.py` to compose/import the moved presentation rather than own it.
