...
class DuplicateReviewDialog(QDialog):
    ...
    def show_group(self,item):
        ...
        for rank_no,r in enumerate(members,1):
            ...
            eligibility_text='可直接用' if r.eligibility=='PASS' else ('需复核' if r.eligibility=='REVIEW' else '硬淘汰');it=QListWidgetItem(QIcon(self.thumb(r.path)),text);it.setData(Qt.UserRole,index);it.setFlags(it.flags()|Qt.ItemIsUserCheckable);it.setCheckState(Qt.Checked if checked else Qt.Unchecked);it.setToolTip(f'{r.sample_id}\n文件：{human_bytes(r.file_size)} · {r.width}×{r.height}\n状态：{r.status} · 判定：{eligibility_text}');self.members.addItem(it)
